 ...
 * ****************************************************************************
 *
 * Implementation of the external propagator for CaDiCaL SAT Solver.
 */
...
void CadicalPropagator::notify_assignment(int lit, bool is_fixed)
{
  ...
  Trace("cadical::propagator") << "notif::assignment: ["
                               << (is_decision ? "d" : "p") << "] " << slit
                               << " (fixed: " << is_fixed << ", level: "
                               << d_decisions.size() << ", level_intro: "
                               << info.level_intro << ", level_user: "
                               << current_user_level() << ")" << std::endl;
...
int CadicalPropagator::cb_add_reason_clause_lit(int propagated_lit)
{
  ...
  if (!d_processing_reason)
  {
    ...
    Trace("cadical::propagator") << "cb::reason: " << slit << ", size: "
                                 << d_reason.size() << std::endl;
  }
...
const std::vector<SatLiteral>& CadicalPropagator::get_decisions() const
{
  return d_decisions;
}
...
bool CadicalPropagator::done() const
{
  ...
  Trace("cadical::propagator") << "done: "
                               << (d_found_solution ?
                                   "found partial solution" :
                                   "full assignment consistent")
                               << std::endl;
  return true;
}

void CadicalPropagator::user_push(SatVariable& alit)
{
...
void CadicalPropagator::user_pop()
{
  ...
  while (it != d_assignments.end())
  {
    ...
    {
      Trace("cadical::propagator") << "queue renotify: " << lit
                                   << " (level_intro: " << info.level_intro
                                   << ", level_fixed: " << info.level_fixed
                                   << ")" << std::endl;
...
bool CadicalPropagator::is_fixed(SatVariable var) const
{
...
size_t CadicalPropagator::current_user_level()
{
  return d_activation_literals.size();
}

const std::vector<SatLiteral>& CadicalPropagator::activation_literals()
{
